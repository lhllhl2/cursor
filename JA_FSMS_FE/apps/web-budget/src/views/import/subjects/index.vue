...
const [pageService, syncSubjectsService, apiSyncService] = useMultiRequest([
  {
    request: getSubjectsPage,
  },
  {
    request: syncSubjects,
    config: {
      onSuccess: () => {
...
const [Grid, gridApi] = useVbenVxeGrid({
  gridOptions: {
    ...
    importConfig: hasAccessByCodes([PERMISSION_ENUM.ImportSubjectsImport])
      ? {
          types: ['xlsx'],
          modes: ['covering'],
          remote: true,
          async importMethod(options: any) {
            await importSubjects({ ...options }).then((res: any) => {
              gridApi.query();
...
<template>
  <Page auto-content-height :description="$t('import.subjects.description')">
    <template #extra>
      <Button
        type="primary"
        ...
        @click="onApiSync"
        class="mr-2"
        v-access="PERMISSION_ENUM.ImportSubjectsApiSync"
      >
      ...
      <Button
        type="default"
        danger
        :loading="syncSubjectsService?.loading.value"
        @click="onSync"
      >
        {{ $t('common.sync') }}
      </Button>
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <VxeTableExportButton
          :api="exportSubjects"
          v-access="PERMISSION_ENUM.ImportSubjectsExport"
        />
      </template>
