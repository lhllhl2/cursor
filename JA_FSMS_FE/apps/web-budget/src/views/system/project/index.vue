...
const [orgSyncService, pageService] = useMultiRequest([
  {
    request: projectSync,
    config: {
      onSuccess: () => {
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    ...
    treeConfig: {
      parentField: 'parentProjectCode',
      rowField: 'projectCode',
      transform: true,
    },
...
<template>
  <Page
    auto-content-height
    :description="$t('system.systemProject.description')"
  >
    <template #extra>
      <Button
        type="default"
    ...
    <Grid :table-title="$t('system.systemProject.list')">
      <template #toolbar-tools>
        <Button
          type="primary"
          class="mr-2"
          @click="onBatchAuth"
          v-access="PERMISSION_ENUM.SystemProjectBatchAuth"
        >
        ...
        <VxeTableExportButton
          class="mr-2"
          :api="projectExport"
          v-access="PERMISSION_ENUM.SystemProjectExport"
        />
        <VxeTableImportButton
          :api="projectImport"
          :grid-api="gridApi"
          v-access="PERMISSION_ENUM.SystemProjectImport"
        />
      </template>
    </Grid>
  </Page>
