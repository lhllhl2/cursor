...
import { Button, message, Select, Space, Transfer } from 'ant-design-vue';
...
const copyMode = ref<'add' | 'overwrite'>('add');
...
async function handleSubmit() {
  if (!props.fromUserId || targetKeys.value.length === 0) {
    const modeText = copyMode.value === 'add' ? '新增' : '覆盖';
    message.warning(`请选择要${modeText}的用户`);
    return;
  }

  try {
    loading.value = true;
    await (copyMode.value === 'add'
      ? addUserGroupTree({
          fromUserId: props.fromUserId.toString(),
          toUserIds: targetKeys.value.map((id) => id.toString()),
        })
      : copyUserGroupTree({
          fromUserId: props.fromUserId.toString(),
          toUserIds: targetKeys.value.map((id) => id.toString()),
...
<template>
  <div class="user-transfer-container">
    ...
    <div class="copy-mode-selector">
      <span class="copy-mode-label">复制模式：</span>
      <Select
        v-model:value="copyMode"
        :options="copyModeOptions"
        class="copy-mode-select"
      />
    </div>
