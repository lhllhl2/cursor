package com.jasolar.mis.framework.apilog.core.interceptor;
...
public class ApiAccessLogInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (!SpringUtils.isProd()) {
            ...
            if (CollUtil.isNotEmpty(paramMap)) {
                log.info("[preHandle][开始请求 URL({}) 无参数, 非body =>参数({})]", request.getRequestURI(), paramMap.toString());
            }
            String requestBody = ServletUtils.isJsonRequest(request) ? ServletUtils.getBody(request) : null;
            if (CollUtil.isEmpty(queryString) && CharSequenceUtil.isEmpty(requestBody)) {
                log.info("[preHandle][开始请求 URL({}) 无参数]", request.getRequestURI());
            } else {
                log.info("[preHandle][开始请求 URL({}) 参数({})]", request.getRequestURI(),
                        CharSequenceUtil.blankToDefault(requestBody, queryString.toString()));
            }
            // 计时
    ...
    private void printHandlerMethodPosition(HandlerMethod handlerMethod) {
        ...
        try {
            ...
            List<String> clazzContents = FileUtil
                    .readUtf8Lines(ResourceUtil.getResource(null, clazz).getPath().replace("/target/classes/", "/src/main/java/")
