...
public class MPJLambdaWrapperX<T> extends MPJLambdaWrapper<T> implements ExtWrapper<T, SFunction<T, ?>, MPJLambdaWrapperX<T>> {

    @Override
    public MPJLambdaWrapperX<T> ilikeValue(ExtSqlKeyword keyword, SFunction<T, ?> column, Object val, SqlLike sqlLike) {
        maybeDo(true, () -> appendSqlSegments(columnToSqlSegment(index, alias, column, false), keyword,
