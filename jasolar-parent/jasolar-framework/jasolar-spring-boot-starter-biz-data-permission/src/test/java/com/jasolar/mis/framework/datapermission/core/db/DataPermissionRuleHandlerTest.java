package com.jasolar.mis.framework.datapermission.core.db;

import static com.jasolar.mis.framework.common.util.collection.SetUtils.asSet;
 ...
 * @author zhaohuang
 */
class DataPermissionRuleHandlerTest extends BaseMockitoUnitTest {
    ...
    public void setUp() {
        ...
        DataPermissionRule tenantRule = new DataPermissionRule() {
            ...
            public boolean accept(Table table) {
                return asSet("entity", "entity1", "entity2", "entity3", "t1", "t2", "sys_dict_item", // 支持 MyBatis Plus 的单元测试
                        "t_user", "t_role").contains(table.getName());
            }

            @Override
            public Expression getExpression(Table table) {
                Column column = MyBatisUtils.buildColumn(table.getName(), table.getAlias(), COLUMN);
        ...
        DataPermissionRule deptRule = new DataPermissionRule() {
            ...
            @Override
            public boolean accept(Table table) {
                return asSet("t_user").contains(table.getName());
            }

            @Override
            public Expression getExpression(Table table) {
                Column column = MyBatisUtils.buildColumn(table.getName(), table.getAlias(), COLUMN);
                Long[] deptIds = new Long[] { 10L, 20L };
        ...
        // 设置到上下文
        when(ruleFactory.currentRules()).thenReturn(Arrays.asList(tenantRule, deptRule));
    }
    ...
    @Test
    void update() {
        assertSql("update entity set name = ? where id = ?", "UPDATE entity SET name = ? WHERE id = ? AND entity.tenant_id = 1");
    }
    ...
    void selectSubSelectIn() {
        ...
        // 在最前
        assertSql("SELECT * FROM entity e WHERE e.id IN " + "(select e1.id from entity1 e1 where e1.id = ?) and e.id = ?",
                "SELECT * FROM entity e WHERE e.id IN "
                        + "(SELECT e1.id FROM entity1 e1 WHERE e1.id = ? AND e1.tenant_id = 1) AND e.id = ? AND e.tenant_id = 1");
        // 在最后
        assertSql("SELECT * FROM entity e WHERE e.id = ? and e.id IN " + "(select e1.id from entity1 e1 where e1.id = ?)",
                "SELECT * FROM entity e WHERE e.id = ? AND e.id IN "
                        + "(SELECT e1.id FROM entity1 e1 WHERE e1.id = ? AND e1.tenant_id = 1) AND e.tenant_id = 1");
        // 在中间
        assertSql("SELECT * FROM entity e WHERE e.id = ? and e.id IN " + "(select e1.id from entity1 e1 where e1.id = ?) and e.id = ?",
                "SELECT * FROM entity e WHERE e.id = ? AND e.id IN "
                        + "(SELECT e1.id FROM entity1 e1 WHERE e1.id = ? AND e1.tenant_id = 1) AND e.id = ? AND e.tenant_id = 1");
    }
    ...
    void selectLeftJoin() {
        ...
        assertSql("SELECT * FROM entity e " + "left join entity1 e1 on e1.id = e.id " + "left join entity2 e2 on e1.id = e2.id",
                "SELECT * FROM entity e " + "LEFT JOIN entity1 e1 ON e1.id = e.id AND e1.tenant_id = 1 "
    ...
    void selectRightJoin() {
        // right join
        assertSql("SELECT * FROM entity e " + "right join entity1 e1 on e1.id = e.id",
        ...
        assertSql("SELECT * FROM entity e " + "right join entity1 e1 on e1.id = e.id " + "right join entity2 e2 on e1.id = e2.id ",
                "SELECT * FROM entity e " + "RIGHT JOIN entity1 e1 ON e1.id = e.id AND e.tenant_id = 1 "
    ...
    @Test
    void selectMixJoin() {
        assertSql("SELECT * FROM entity e " + "right join entity1 e1 on e1.id = e.id " + "left join entity2 e2 on e1.id = e2.id",
                "SELECT * FROM entity e " + "RIGHT JOIN entity1 e1 ON e1.id = e.id AND e.tenant_id = 1 "
                        + "LEFT JOIN entity2 e2 ON e1.id = e2.id AND e2.tenant_id = 1 " + "WHERE e1.tenant_id = 1");

        assertSql("SELECT * FROM entity e " + "left join entity1 e1 on e1.id = e.id " + "right join entity2 e2 on e1.id = e2.id",
                "SELECT * FROM entity e " + "LEFT JOIN entity1 e1 ON e1.id = e.id AND e1.tenant_id = 1 "
                        + "RIGHT JOIN entity2 e2 ON e1.id = e2.id AND e.tenant_id = 1 " + "WHERE e2.tenant_id = 1");

        assertSql("SELECT * FROM entity e " + "left join entity1 e1 on e1.id = e.id " + "inner join entity2 e2 on e1.id = e2.id",
                "SELECT * FROM entity e " + "LEFT JOIN entity1 e1 ON e1.id = e.id AND e1.tenant_id = 1 "
    ...
    void selectJoinSubSelect() {
        ...
        assertSql("select * from entity1 e1 " + "left join (select * from entity2) e2 " + "on e1.id = e2.id", "SELECT * FROM entity1 e1 "
                + "LEFT JOIN (SELECT * FROM entity2 WHERE entity2.tenant_id = 1) e2 " + "ON e1.id = e2.id " + "WHERE e1.tenant_id = 1");
    }
    ...
    void selectSubJoin() {
        ...
        assertSql("select * FROM " + "(entity1 e1 right JOIN entity2 e2 ON e1.id = e2.id)",
                "SELECT * FROM " + "(entity1 e1 RIGHT JOIN entity2 e2 ON e1.id = e2.id AND e1.tenant_id = 1) " + "WHERE e2.tenant_id = 1");

        assertSql("select * FROM " + "(entity1 e1 LEFT JOIN entity2 e2 ON e1.id = e2.id)",
                "SELECT * FROM " + "(entity1 e1 LEFT JOIN entity2 e2 ON e1.id = e2.id AND e2.tenant_id = 1) " + "WHERE e1.tenant_id = 1");

        assertSql("select * FROM " + "(entity1 e1 LEFT JOIN entity2 e2 ON e1.id = e2.id) " + "right join entity3 e3 on e1.id = e3.id",
        ...
        assertSql("select * FROM entity e " + "LEFT JOIN (entity1 e1 right join entity2 e2 ON e1.id = e2.id) " + "on e.id = e2.id",
                "SELECT * FROM entity e " + "LEFT JOIN (entity1 e1 RIGHT JOIN entity2 e2 ON e1.id = e2.id AND e1.tenant_id = 1) "
                        + "ON e.id = e2.id AND e2.tenant_id = 1 " + "WHERE e.tenant_id = 1");

        assertSql("select * FROM entity e " + "LEFT JOIN (entity1 e1 left join entity2 e2 ON e1.id = e2.id) " + "on e.id = e2.id",
                "SELECT * FROM entity e " + "LEFT JOIN (entity1 e1 LEFT JOIN entity2 e2 ON e1.id = e2.id AND e2.tenant_id = 1) "
                        + "ON e.id = e2.id AND e1.tenant_id = 1 " + "WHERE e.tenant_id = 1");

        assertSql("select * FROM entity e " + "RIGHT JOIN (entity1 e1 left join entity2 e2 ON e1.id = e2.id) " + "on e.id = e2.id",
                "SELECT * FROM entity e " + "RIGHT JOIN (entity1 e1 LEFT JOIN entity2 e2 ON e1.id = e2.id AND e2.tenant_id = 1) "
    ...
    void selectLeftJoinMultipleTrailingOn() {
        // 多个 on 尾缀的
        assertSql(
                "SELECT * FROM entity e " + "LEFT JOIN entity1 e1 " + "LEFT JOIN entity2 e2 ON e2.id = e1.id " + "ON e1.id = e.id "
                        + "WHERE (e.id = ? OR e.NAME = ?)",
                "SELECT * FROM entity e " + "LEFT JOIN entity1 e1 " + "LEFT JOIN entity2 e2 ON e2.id = e1.id AND e2.tenant_id = 1 "
                        + "ON e1.id = e.id AND e1.tenant_id = 1 " + "WHERE (e.id = ? OR e.NAME = ?) AND e.tenant_id = 1");

        assertSql(
                "SELECT * FROM entity e " + "LEFT JOIN entity1 e1 " + "LEFT JOIN with_as_A e2 ON e2.id = e1.id " + "ON e1.id = e.id "
                        + "WHERE (e.id = ? OR e.NAME = ?)",
    ...
    void selectInnerJoin() {
        ...
        assertSql("SELECT * FROM entity e " + "inner join entity1 e1 on e1.id = e.id " + "WHERE e.id = ? OR e.name = ?",
                "SELECT * FROM entity e " + "INNER JOIN entity1 e1 ON e1.id = e.id AND e.tenant_id = 1 AND e1.tenant_id = 1 "
                        + "WHERE e.id = ? OR e.name = ?");
        ...
        // 隐式内连接
        assertSql("SELECT * FROM entity a, with_as_entity1 b " + "WHERE a.id = b.id",
                "SELECT * FROM entity a, with_as_entity1 b " + "WHERE a.id = b.id AND a.tenant_id = 1");

        assertSql("SELECT * FROM with_as_entity a, with_as_entity1 b " + "WHERE a.id = b.id",
        ...
        assertSql("SELECT * FROM ((entity,entity1),entity2) " + "WHERE entity.id = entity1.id and entity.id = entity2.id",
    ...
    void selectIgnoreTable() {
        assertSql(
                " SELECT dict.dict_code, item.item_text AS \"text\", item.item_value AS \"value\" FROM sys_dict_item item INNER JOIN sys_dict dict ON dict.id = item.dict_id WHERE dict.dict_code IN (1, 2, 3) AND item.item_value IN (1, 2, 3)",
    ...
    void testSelectLeftJoin() {
        // left join
        assertSql("SELECT * FROM t_user e " + "left join t_role e1 on e1.id = e.id " + "WHERE e.id = ? OR e.name = ?",
                "SELECT * FROM t_user e " + "LEFT JOIN t_role e1 ON e1.id = e.id AND e1.tenant_id = 1 "
    ...
    void testSelectRightJoin() {
        // right join
        assertSql("SELECT * FROM t_user e " + "right join t_role e1 on e1.id = e.id " + "WHERE e.id = ? OR e.name = ?",
                "SELECT * FROM t_user e " + "RIGHT JOIN t_role e1 ON e1.id = e.id AND e.tenant_id = 1 AND e.dept_id IN (10, 20) "
                        + "WHERE (e.id = ? OR e.name = ?) AND e1.tenant_id = 1");
    ...
    void testSelectInnerJoin() {
        // inner join
        assertSql("SELECT * FROM t_user e " + "inner join entity1 e1 on e1.id = e.id " + "WHERE e.id = ? OR e.name = ?",
