...
/**
 * 预算季度聚合视图 Mapper（按采购额/付款额分类）
 * 对应视图：V_BUDGET_QUARTERLY_AGGREGATE_BY_TYPE
 *
 * @author Auto
 */
@Mapper
public interface BudgetQuarterlyAggregateByTypeMapper extends BaseMapperX<BudgetQuarterlyAggregateByType> {

    /**
     * 分页查询预算季度聚合数据（按采购额/付款额分类）
     *
     ...
     * @return 数据列表
     */
    List<BudgetQuarterlyAggregateByType> selectPageByParams(
    ...
    /**
     * 查询预算季度聚合数据列表（按采购额/付款额分类）
     *
     * @param year 年度（可选）
     * @param ehrCd EHR组织编码（可选）
     * @param erpAssetType ERP资产类型编码（可选）
     * @return 数据列表
     */
    List<BudgetQuarterlyAggregateByType> selectList(
            @Param("year") String year,
            @Param("ehrCd") String ehrCd,
     ...
     * @return 数据列表
     */
    List<BudgetQuarterlyAggregateByType> selectListByParams(
            @Param("params") com.jasolar.mis.module.system.controller.budget.vo.BudgetAssetBalanceQueryParams params
    );
}
