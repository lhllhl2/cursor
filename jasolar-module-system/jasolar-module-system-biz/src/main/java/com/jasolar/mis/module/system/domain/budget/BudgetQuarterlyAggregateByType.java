...
/**
 * 预算季度聚合视图实体（按采购额/付款额分类）
 * 对应视图：V_BUDGET_QUARTERLY_AGGREGATE_BY_TYPE
 * 说明：按采购额和付款额分类展示预算季度聚合数据，按YEAR、EHR_CD、ERP_ASSET_TYPE聚合
 *
 * @author Auto
 */
@Data
public class BudgetQuarterlyAggregateByType {
    ...
    /**
     * 去年使用预算数（采购额）：去年同一维度的 PURCHASE_Q4_AMOUNT_FROZEN + PURCHASE_Q4_AMOUNT_OCCUPIED + PURCHASE_Q4_AMOUNT_ACTUAL
     */
    private BigDecimal purchaseLastYearUsage;

    /**
     * 去年使用预算数（付款额）：去年同一维度的 PAYMENT_Q4_AMOUNT_FROZEN + PAYMENT_Q4_AMOUNT_OCCUPIED + PAYMENT_Q4_AMOUNT_ACTUAL
     */
    private BigDecimal paymentLastYearUsage;
